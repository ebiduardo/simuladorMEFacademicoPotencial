...
!
    ...
    subroutine readStringKeywordValue(keyword, target, defaultValue, ierr)
        implicit none
        character(80), intent(in) :: keyword
        ...
        integer*4 keyword_line
        character(120) file_line
        character(80) :: origem = 'valor default, ' 
        !character(len=*),parameter :: free='(*(g0,1x))'
        !print free, '1:', trim(keyword),', 2:', trim(target),', 3:', trim(defaultValue),', 4:', ierr
...
!       print*, value
        ...
        write(*,'(a, a, a, a)') trim(origem), trim(keyword), '=', trim(target)
