...
!
      integer*4 :: nen,numnp,numel
      integer*4, dimension(:,:), target  :: conecElem
      integer*4, dimension(:,:), target  :: listaDosElemsPorNo
      integer*4:: no,nel,l

      print*, 'em criarListaVizinhos, I', numel, nen, numnp
      do nel=1, numel
         do l=1, nen
            no=conecElem(l,nel)
...
!     Objetivo: cria a matriz listaDosElemsPorNo: .... indices dos elementos que possuem o no
      integer nen,numnp,numel,nVizinMax
      integer, dimension(nen,numel)  :: conecElem
      integer, pointer  :: listaDosElemsPorNo(:,:)!(nVizinMax,numnp)
      integer, pointer  :: lmStencilEq(:,:) !(nVizinMax,numnp)
