...
!
      ...
      integer :: passo;  real*8 :: tempo

      write(*,*) "em processamento A "
...
!    if(estrutSistEq_%ndof == nsd) then ! fluxo 
 ...
 ! cg + jacobi preconditioner
     ...
     call timing(tB)
      ...
      ! call escreverSolucaoB(estrutSistEq_, r) 
  ...
  subroutine escreverSolucaoP(estrutSistEq_)
      ...
      integer*4 :: i
      real*8    :: solutionNorm
      write(*,*) " valores nos extremos do vetor solucao completo,  ", trim(label), r
      write(*,'(7e12.4)') estrutSistEq_%u(1, 1    :6+1)
      write(*,'(7e12.4)') estrutSistEq_%u(1, 1+7  :6+1+7)
      write(*,'(7e12.4)') estrutSistEq_%u(1, numnp-(6+1-1): numnp)
      solutionNorm = 0.0 
        do i = 1, numnp
  ...
  subroutine escreverSolucaoF(estrutSistEq_)
      ...
      write(*,*) " valores nos extremos do vetor solucao completo,  ", trim(label), r
      write(*,'(8e12.4)') estrutSistEq_%u(1:ndof, 1    :4)
      write(*,'(8e12.4)') estrutSistEq_%u(1:ndof, numnp-3: numnp)
      solutionNorm = 0.0 
        do j = 1, nsd
         do i = 1, numnp
